...
var _ref,
	...
	__isIos = /(iPad|iPhone|iPod)/g.test(navigator.userAgent),
	__isAndroid = /(Android)/g.test(navigator.userAgent),
...
// 	}
// };
//Prevent Trigger click when keys are pressed
vjs.Button.prototype.onKeyPress = function (event) {
};
//adapt playbackrates values to videojs
	...
	function vjsAnnotation_ (options) {
		...
		function initialVideoFinished (event) {
			...
			var wrapper = $('.annotator-wrapper').parent()[0],
				...
				rangeslider = player.rangeslider;
			...
			if (!options.seeActionButtons)
				plugin.removeActiveButtons();
			...
			player.rangeslider.rstb.on('mousedown', function (event) {
				plugin._onMouseDownRS(event)
			});
			...
			$(player.el_).watch('width', function () {
				if (player.isFullscreen()) {
					$(player.annotator.wrapper[0]).addClass('vjs-fullscreen');
				} else {
					$(player.annotator.wrapper[0]).removeClass('vjs-fullscreen');
				}
				//restart the scroll top of the anpanel
				plugin.AnDisplay.el_.scrollTop = 0;
				plugin._setOverRS(player.annotator.editor.element);
				if(!player.annotator.editor.element.hasClass('annotator-hide')){
					player.annotator.editor.checkOrientation();
				}
				plugin.refreshDesignPanel();
			});
...
//-- Plugin
	function vjsAnnotation (player, options) {
		...
		if (!options.hasOwnProperty('seeActionButtons'))
			options.seeActionButtons = true;
...
//-- Methods
	vjsAnnotation.prototype = {
		...
		init: function () {
			...
			//local variables
			this.editing = false;
			this.preventTinymceFocus = false; //prevent do a focus on tinymce every time it is created an annotation
			...
			annotator.subscribe("annotationsLoaded", function (annotations) {
				if (self.loaded) {
					self.refreshDisplay();
					self.AnStat.paintCanvas();//refresh canvas
				}
			});
		...
		newan: function (start, end) {
			...
			annotator.selectedRanges = []; //fix an issue with annotator 1.0
		...
		removeActiveButtons: function () {
			$(this.AnConBut.el_).remove();
		},
		...
		refreshDesignPanel: function () {
			...
			this.BackAnDisplay.el_.style.bottom = this.backDSBar.el_.style.bottom = (controlBarHeight + 'px');
		},
		_reset: function () {
		...
		_focus_tinymce: function () {
			var ed = tinyMCE.get('annotator-field-0');
			setTimeout(function () {
				//select all
		...
		_onMouseDownRS: function (event) {
			...
			//create a protection in the video for iframe
			var player = this.player, self = this;
			var el = $(player.el()).find('.vjs-control-bar').before('<div id="vjs-control-bar" class="vjs-control-bar"></div>');
			el = $(player.el()).find('#vjs-control-bar.vjs-control-bar');
			el.css('width', $(player.el()).css('width'));
			el.css('background-color', 'rgba(7,20,30,.0)');
			el.css('height', $(player.el()).css('height'));
		...
		_onMouseUpRS: function (event) {
			...
			//remove protection for iframe of youtube
			var player = this.player;
			var el = $(player.el()).find('#vjs-control-bar.vjs-control-bar');
			el.remove();
...
OpenVideoAnnotation.Annotator = function (element, options) {
	...
	if (typeof options.optionsAnnotator.highlightTags != 'undefined')
		...
		this.annotator.addPlugin("Tags", options.optionsAnnotator.Tags);
